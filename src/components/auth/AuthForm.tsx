...
export function AuthForm({ className, ...props }: UserAuthFormProps) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
    if(email.length < 6) return toast("Email must be at least 6 characters long", {icon: "⚠️"})
    if(password.length < 6) return toast("Password must be at least 6 characters long", {icon: "⚠️"})
  ...
  async function loginWithGoogle() {
    alert("Currently Out of Service! Use Standard Method Please");
  }
